---
title: "Neural Network dengan Backpropagation untuk Meramalkan Pertumbuhan Ekspor"
output:
...
Data time-series (bulanan) tersebut dapat divisualisasikan sebagai berikut:
...
Dalam membentuk model NN dengan Backpropagation atau BPNN dan melakukan peramalan, data terlebih dahulu dibagi menjadi training set dan testing set. Splitting point untuk membagi data tersebut misalnya pada bulan Februari 2021.
...
Selanjutnya, untuk melakukan BPNN data terlebih dahulu data dinormalisasi.
...
Untuk meramalkan nilai pertumbuhan ekspor ke depannya dengan menggunakan pendekatan NN, diperlukan input variabel. Berbekal data time series yang dimiliki, dilakukan transformasi sehingga menghasilkan beberapa input variabel yang digunakan dalam NN. Proses transformasi ini dilakukan dengan menggunakan bantuan package `maltese`. 

Idenya adalah dengan mengekstrak beberapa variabel waktu (e.g. dalam pengamatan ini adalah periode bulan dan jumlah observasi sebelumnya), menjadikannya sebagai *new features*, kemudian mengubahnya dalam bentuk faktor dan mengonversinya dalam bentuk dummy. 
...
Selanjutnya kita melatih model dengan berbekal data latih sebanyak 61 pengamatan berdasarkan splitting date.  
...
Disimulasikan 4 model NN dengan mengkombinasikan jumlah hidden layer dan banyaknya layer, yaitu (1) [5,3]; (2) [7,5,3]; (3) [7,5,5,3]; (4) [7,7,5,3]. Seluruh model menggunakan learning rate sebesar 0.01 dengan fungsi algoritma **Backpropagation**. Dari setiap model akan dievaluasi nilai error dari modelnya **dan** indikator akurasi prediksi yang umum digunakan dalam time series model yaitu root mean squared error (RMSE) serta mean absolute error (MAE). Model yang memberikan error terkecil akan dipilih sebagai model terbaik. 
...
Error dari masing-masing model sebagai berikut:
...
Dilihat dari error, Model 3 dan 4 memiliki nilai terkecil. Langkah selanjutnya adalah membandingkat tingkat ketepatan model dalam meramalkan nilai aktual
...
Kemudian dapat kita bandingkan plot nya sebagai berikut:
...
Setelah plot tersebut, evaluasi atas keakuratan prediksi sebagai berikut:
...
Dari perbandingan tersebut dapat dilihat bahwa Model 3 memiliki nilai RMSE dan MAE paling kecil. Dengan demikian Model 3 [7,5,5,3] dipilih sebagai model untuk melakukan peramalan. Model Neural Network yang dibentuk dapat ditampilkan sebagai berikut:
...
Berdasarkan model BPNN yang dibentuk, misalkan dilakukan peramalan untuk 12 periode waktu ke depan. 
...
Hasil peramalan tersebut apabila diplot sebagai berikut:
...
Nilai dan plot 12 pengamatan hasil peramalan sebagai berikut:

```{r}

# data
mynewdata[, 1:2] %>% filter(date > "2022-03-01") %>%
  kbl() %>%
  kable_paper(full_width = T)

# plot
ggplot(filter(mynewdata, date > "2022-03-01"),
       aes(x = date, y = export_growth, 
